import { db } from '~/server/utils/db'

interface WorkspaceRow { id: string; owner_id: string }
interface TokenRow { id: string; workspace_id: string }

export default defineEventHandler((event) => {
  const user = event.context.user
  if (!user) throw createError({ statusCode: 401, message: 'Unauthorized' })

  const workspaceId = getRouterParam(event, 'id')
  const tokenId = getRouterParam(event, 'tokenId')

  const workspace = db.prepare('SELECT id, owner_id FROM workspaces WHERE id = ?').get(workspaceId) as WorkspaceRow | undefined
  if (!workspace) throw createError({ statusCode: 404, message: 'Workspace not found' })
  if (workspace.owner_id !== user.id) throw createError({ statusCode: 403, message: 'Forbidden' })

  const apiToken = db.prepare('SELECT id, workspace_id FROM api_tokens WHERE id = ?').get(tokenId) as TokenRow | undefined
  if (!apiToken || apiToken.workspace_id !== workspaceId) {
    throw createError({ statusCode: 404, message: 'Token not found' })
  }

  db.prepare('DELETE FROM api_tokens WHERE id = ?').run(tokenId)
  return { success: true }
})
