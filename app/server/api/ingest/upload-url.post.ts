...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody(event).catch(() => ({})) as { originalName?: string; reporter_email?: string; reporter_name?: string }
  ...
  db.prepare(`
    INSERT INTO reels (id, workspace_id, filename, original_name, size, created_at, app_id, uploaded_by_user_id, reporter_email, reporter_name)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, app.workspace_id, filename, body.originalName ?? null, null, now, app.id, apiToken.user_id, body.reporter_email?.trim() || null, body.reporter_name?.trim() || null)
